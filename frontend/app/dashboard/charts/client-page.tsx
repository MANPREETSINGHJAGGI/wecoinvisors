  ...
  return (
    <div className="p-6 text-white">
      ...
      <div className="flex gap-4 mb-4">
        <select value={symbol} onChange={(e) => setSymbol(e.target.value)} className="bg-dark border border-gold px-3 py-1 rounded">
          {SYMBOLS.map((s) => (
            <option key={s} value={s}>{s}</option>
          ))}
        </select>

        <select value={range} onChange={(e) => setRange(e.target.value)} className="bg-dark border border-gold px-3 py-1 rounded">
          {RANGES.map((r) => (
            <option key={r} value={r}>{r}</option>
          ))}
        </select>

        <button onClick={downloadPNG} className="bg-gold text-black px-3 py-1 rounded shadow-gold">Export PNG</button>
        <button onClick={downloadCSV} className="bg-gold text-black px-3 py-1 rounded shadow-gold">Export CSV</button>
      </div>
